...
2. Columns: one per title, equal width, sharing the full width. Everything in a column is centered
   in it: poster, title, the add slot and each row's value. With fewer than 3 titles, the next column is an "Add a
