...
You can only learn facts by calling the tools. You have at most 4 tool calls.
...
- If a query returns no rows, use status "no_data" and say which condition had no data.
...
def numbers_are_grounded(answer: str, evidence: list[m.Evidence], context: str) -> bool:
    """Each number in the answer must match a tool result, allowing display rounding."""
...
def answer(messages: list[m.ChatMessage]) -> m.AssistantReply:
    ...
    if status == "answered":
        ...
        if not succeeded or not numbers_are_grounded(text, succeeded, context):
            logger.warning("assistant answer rejected: not grounded in tool results")
            return reply("failed", None, evidence)
